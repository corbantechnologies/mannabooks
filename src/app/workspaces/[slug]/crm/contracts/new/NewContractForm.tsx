...
export function NewContractForm({ shopId, shopSlug, clients, deals, currency, brandColor, initialDealId }: NewContractFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      <Link href={`/workspaces/${shopSlug}/crm/contracts`} className="inline-flex items-center gap-1.5 text-sm text-gray-500 hover:text-gray-700 dark:hover:text-gray-300">
      ...
      <div className="rounded-xl border border-gray-200 dark:border-white/8 bg-white dark:bg-white/3 p-6 space-y-4">
        <h2 className="text-sm font-semibold text-gray-800 dark:text-white">Contract Details</h2>
        <div>
      ...
      <div className="rounded-xl border border-gray-200 dark:border-white/8 bg-white dark:bg-white/3 p-6 space-y-4">
        <h2 className="text-sm font-semibold text-gray-800 dark:text-white">Billing</h2>
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className={`flex-1 py-2 rounded-lg text-sm font-medium border transition-all ${!isHoursBased ? "border-transparent text-white" : "border-gray-200 dark:border-white/10 text-gray-500"}`}
        ...
        <div className="flex items-center gap-3 p-3 rounded-lg bg-gray-50 dark:bg-white/4">
          ...
          />
          <label htmlFor="auto-invoice" className="text-sm text-gray-700 dark:text-gray-300 cursor-pointer">
      ...
      <div className="rounded-xl border border-gray-200 dark:border-white/8 bg-white dark:bg-white/3 p-6 space-y-4">
        <h2 className="text-sm font-semibold text-gray-800 dark:text-white">Term</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
          ...
          <div>
            <label className={labelClass}>End Date <span className="text-gray-400">(leave blank for open-ended)</span></label>
      ...
      <button type="submit" disabled={isPending} className="w-full py-3 rounded-xl text-white font-semibold text-sm disabled:opacity-60" style={{ backgroundColor: brandColor }}>
