...
export default async function DealDetailPage({ params }: DealDetailPageProps) {
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 md:p-6 space-y-6">
      {/* Back */}
      <Link href={`/workspaces/${slug}/crm`} className="inline-flex items-center gap-1.5 text-sm text-gray-600 hover:text-gray-900 transition-colors">
      ...
      <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Contact: <strong className="text-gray-900">{deal.contactName}</strong></p>
        </div>
        <div className="flex flex-wrap gap-2 shrink-0">
          <Link
            href={`/workspaces/${slug}/crm/proposals?dealId=${deal.id}`}
            className="flex items-center gap-1.5 px-3 py-2 text-sm border border-gray-200 rounded-lg text-gray-700 bg-white hover:bg-gray-50 shadow-sm transition-colors"
          >
          ...
          <div className="rounded-xl border border-gray-200 bg-white shadow-sm p-5 space-y-3">
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm font-semibold text-gray-900">{deal.winProbability}%</span>
              </div>
          ...
          <div className="rounded-xl border border-gray-200 bg-white shadow-sm p-5 space-y-3">
            ...
            <div className="space-y-2">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm text-gray-800">{deal.contactName}</span>
              </div>
              {deal.contactEmail && (
                <div className="flex items-center gap-2">
                  <Mail className="w-3.5 h-3.5 text-gray-400 shrink-0" />
                  <a href={`mailto:${deal.contactEmail}`} className="text-sm text-blue-600 hover:underline truncate">{deal.contactEmail}</a>
                </div>
              )}
              {deal.contactPhone && (
                <div className="flex items-center gap-2">
                  <Phone className="w-3.5 h-3.5 text-gray-400 shrink-0" />
                  <a href={`tel:${deal.contactPhone}`} className="text-sm text-gray-800">{deal.contactPhone}</a>
                </div>
              )}
              {expectedCloseDate && (
                <div className="flex items-center gap-2">
                  <Calendar className="w-3.5 h-3.5 text-gray-400 shrink-0" />
                  <span className="text-sm text-gray-800">{expectedCloseDate}</span>
                </div>
              )}
            </div>
          </div>
            ...
            <div className="rounded-xl border border-gray-200 bg-white shadow-sm p-5 space-y-3">
              ...
              <div className="space-y-2">
                {deal.proposals.map((p: any) => (
                  ...
                  >
                    <span className="text-xs font-medium text-gray-800">{p.proposalNumber}</span>
          ...
          {deal.contracts && deal.contracts.length > 0 && (
            <div className="rounded-xl border border-gray-200 bg-white shadow-sm p-5 space-y-3">
              ...
              {deal.contracts.map((c: any) => (
                <Link key={c.id} href={`/workspaces/${slug}/crm/contracts/${c.id}`} className="flex items-center justify-between p-2.5 rounded-lg bg-gray-50 hover:bg-gray-100 border border-gray-100">
                  <span className="text-xs font-medium text-gray-800">{c.contractNumber}</span>
                  <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-emerald-100 text-emerald-700 font-medium">{c.status}</span>
                </Link>
              ))}
            </div>
          )}
      ...
      {deal.notes && (
        <div className="rounded-xl border border-gray-200 bg-white shadow-sm p-5">
          ...
          <p className="text-sm text-gray-800 whitespace-pre-line leading-6">{deal.notes}</p>
        </div>
      )}
    </div>
  );
