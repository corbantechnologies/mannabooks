...
export function NewProposalForm({
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      <Link href={`/workspaces/${shopSlug}/crm/proposals`} className="inline-flex items-center gap-1.5 text-sm text-gray-600 hover:text-gray-900 transition-colors">
      ...
      {/* Narrative sections */}
      <div className="rounded-xl border border-gray-200 bg-white shadow-sm p-6 space-y-4">
        <h2 className="text-sm font-bold text-gray-900">Narrative</h2>
        <div>
      ...
      <div className="rounded-xl border border-gray-200 bg-white shadow-sm p-6 space-y-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <span className="text-xs text-gray-600 font-medium">Multi-tier packages?</span>
            <button
              type="button"
              onClick={() => setIsMultiTier(v => !v)}
              className={`relative w-9 h-5 rounded-full transition-colors ${isMultiTier ? "bg-emerald-600" : "bg-gray-200"}`}
            >
        ...
        <div className="space-y-3">
          {items.map((item, i) => (
            <div key={i} className="grid gap-2 p-3 rounded-lg border border-gray-200 bg-gray-50">
              <div className="grid grid-cols-1 md:grid-cols-[1fr_80px_120px] gap-2">
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-white border border-gray-300 rounded-md px-3 py-2 text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-emerald-500"
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-white border border-gray-300 rounded-md px-3 py-2 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-emerald-500"
                  />
                </div>
                <div>
              ...
              {isMultiTier && (
                <div>
                  <label className="block text-[11px] font-semibold text-gray-600 mb-1">Package / Tier Name</label>
                  <input
                    type="text"
              ...
              <div className="flex items-center justify-between">
                <p className="text-xs text-gray-600 font-medium">
                  Total: <strong className="text-gray-900">{form.currency} {getLineTotal(item).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</strong>
                </p>
                {items.length > 1 && (
